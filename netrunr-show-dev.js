/*
 * Example program - Show connected BLE devices - single shot mode
 *
...
async function main () {
  ...
  console.log(`Found ${gwlist.length} Gateways`)
  const devInfo = []
  ...
  await Promise.all(gwlist.map(async gw => {
    if (!gNetrunrClient.getBleLinkStatus(gw.id)) {
      gwHandleList[gw.id] = await gNetrunrClient.createBleLink(gw.id)
      if (gNetrunrClient.getBleLinkStatus(gw.id)) {
